...
export function resolveBoutiques(model, boutiquesDict, province) {
  const esCmp = (a, b) => String(a || '').localeCompare(String(b || ''), 'es');
  const list = model.boutiques
    .map((b) => ({ cod: b.cod, colors: b.colors, ...(boutiquesDict[b.cod] || {}) }))
    .filter((b) => b.name);
  const filtered = province ? list.filter((b) => b.province === province) : list;
